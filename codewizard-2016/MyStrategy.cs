...
namespace Com.CodeGame.CodeWizards2016.DevKit.CSharpCgdk
{
	public sealed class MyStrategy : IStrategy
	{
		...
		private void AnalyzeActions()
		{
			...
			if (LastAction.Value == ActionType.Fireball)
				CanFireHit = _self.RemainingCooldownTicksByAction[(int)ActionType.Fireball] > 0;
			else if (LastAction.Value == ActionType.FrostBolt)
				CanFireHit = _self.RemainingCooldownTicksByAction[(int)ActionType.FrostBolt] > 0;
			else if (LastAction.Value == ActionType.Haste)
				CanFireHit = _self.RemainingCooldownTicksByAction[(int)ActionType.Haste] > 0;
			else if (LastAction.Value == ActionType.Shield)
				CanFireHit = _self.RemainingCooldownTicksByAction[(int)ActionType.Shield] > 0;
		}
		...
		private void AnalyzeBaseState()
		{
			...
			if (_self.Life < _self.MaxLife*0.2)
			{
		...
		private void AnalyzeTargets()
		{
			...
			TargetsToMagicAttack =
				AllTargets.Where(
					t =>
						...
						Abs(_self.GetAngleTo(t)) < _game.StaffSector / 2)
					.OrderBy(m => m.Life)
					.ThenBy(m => m.GetDistanceTo(_self)).ToArray();
			TargetsToFireAttack = TargetsToMagicAttack.Where(t => t is Building || t is Wizard).OrderBy(m => m.Life).ToArray();
			TargetsToFrostAttack = TargetsToMagicAttack.Where(t => t is Wizard).OrderBy(m => m.Life).ToArray();
			...
			NearestUnits =
				AllTargets.Concat(AllTeam)
					...
					.ToArray();
			ProjectilesAimedToMe =
				_world.Projectiles.Where(
					p => p.Faction != _self.Faction && 
						Abs((p.SpeedY/p.SpeedX*_self.X - _self.Y + p.Y)/Sqrt(p.SpeedY/p.SpeedX*p.SpeedY/p.SpeedX + 1)) <
						p.Radius + _self.Radius && Abs(p.GetAngleTo(_self)) < PI/2)
					.OrderBy(p => p.GetDistanceTo(_self))
					.ThenBy(p => Abs(p.GetAngleTo(_self)))
					.ToArray();
			...
			_alert |= AllTargets.Length > AllTeam.Length || TargetsAimedToMe.Length > 1;
		}
		...
		{
			if (ProjectilesAimedToMe.Any() && TryDodge())
			{
				return;
			}
		...
		private void SetActionStrategy()
		{
			...
			_move.Action = CanSpecialAct(ActionType.Shield)
				...
				: CanSpecialAct(ActionType.Haste)
					? ActionType.Haste
					: CanSpecialAct(ActionType.Fireball) &&
					  TargetsToFireAttack != null && TargetsToFireAttack.Any()
						? ActionType.Fireball
						: CanSpecialAct(ActionType.FrostBolt) &&
							  ...
							  TargetsToMagicAttack != null && TargetsToMagicAttack.Any()
								...
								: CanSpecialAct(ActionType.Staff) &&
								  (TargetsToStaffAttack != null && TargetsToStaffAttack.Any())
									? ActionType.Staff
									: ActionType.None;
			LastAction = _move.Action;
			_move.StatusTargetId = _self.Id;
		}
		...
		private bool IsUnitOnWay(KeyPoint startPoint, KeyPoint finishPoint, double radius)
		{
			...
			{
				if (startPoint.GetDistanceTo(u) + finishPoint.GetDistanceTo(u) > 1.2*startPoint.GetDistanceTo(finishPoint))
					return false;
		...
		private KeyPoint[] FindMoveOutPointFromProjectiles(Projectile projectile)
		{
		...
		{
			if (!ProjectilesAimedToMe.Any()) return false;
			var targetProjectile = ProjectilesAimedToMe.First();
			var possibleMovePoints =
				FindMoveOutPointFromProjectiles(targetProjectile)
					.Where(p => !IsUnitOnWay(SelfPoint, p, _self.Radius))
					.OrderBy(p => p.GetDistanceTo(_self))
					.ToArray();
			if (!possibleMovePoints.Any()) return false;
			CurrentRoute = BuildRoute(_self, possibleMovePoints.First());
			return true;
		}
		...
		private void MoveTo(double x, double y, bool invert = false, bool withTurn = true, bool invertTurning = false)
		{
			...
			var strafe = (angle < 0 ? invert ? 1 : -1 : invert ? -1 : 1) *
						 Sqrt(1 - Abs(speed) / (speed > 0 ? _game.WizardForwardSpeed : _game.WizardBackwardSpeed)) *
		...
		{
			return (_self.RemainingCooldownTicksByAction[(int)action] == 0 &&
										  (action == ActionType.FrostBolt && _self.Mana > _game.FrostBoltManacost && CanFireHit != null && CanFireHit.Value ||
										   action == ActionType.Fireball && _self.Mana > _game.FireballManacost && CanFrosenHit != null && CanFrosenHit.Value
										   || action == ActionType.Shield && _self.Mana > _game.ShieldManacost && CanShield != null && CanShield.Value
										   || action == ActionType.Haste && _self.Mana > _game.HasteManacost && CanHaste != null && CanHaste.Value
		...
		private bool? CanFireHit { get; set; }
		private bool? CanFrosenHit { get; set; }
		private bool? CanHaste { get; set; }
		private bool? CanShield { get; set; }
		private ActionType? LastAction { get; set; }
		...
		private Projectile[] ProjectilesAimedToMe { get; set; }
